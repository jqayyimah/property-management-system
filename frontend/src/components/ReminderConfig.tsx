...
type ReminderConfigVariant = 'full' | 'settings' | 'template';
...
export default function ReminderConfig({
  ...
  const resetTemplateViewport = () => {
    requestAnimationFrame(() => {
      ...
      templateRef.current.scrollTop = 0;
      templateRef.current.scrollLeft = 0;
      templateRef.current.setSelectionRange(0, 0);
    });
  };
  ...
  }, [showSettings, showTemplate]);

  const handleSave = async () => {
    setSaving(true);
    setStatus('idle');
    try {
      if (showSettings && channels.length === 0) {
        setStatus('error');
      ...
      if (showTemplate) {
        saves.push(saveMessageTemplate(template));
      }
      if (showSettings) {
        saves.push(saveReminderChannels(channels));
        saves.push(saveReminderSchedule(scheduleRules));
      }

      await Promise.all(saves);

      if (showTemplate) {
        const formatted = formatTemplateForEditor(template);
        setTemplate(formatted);
        setOriginal(formatted);
        resetTemplateViewport();
      }
    ...
    <div className="config-card">
      {showSettings && (
        <>
          ...
          {showTemplate && <div className="divider" />}
        </>
      )}
